"""
Top 10 RQVI–Flashier pairs: UMAP + MD plots for each pair, plus
...
filtered_df = filtered_df.sort_values("best_corr", ascending=False).reset_index(drop=True)
print(f"Filtered to {len(filtered_df)} pairs with robust RQVI GPs (match_count >= 5)")
top10 = filtered_df.iloc[:10].copy()
print("Top 10 pairs:")
...
for _, row in top10.iterrows():
    PAIRS.append({
        "flashier_factor": row["flashier_factor"],     # e.g. "F21"
...
ax_hist.set_title("Cluster-level similarity (n=200) — top 10 pairs (robust GPs only, match_count≥5)",
...
# ─── Save ────────────────────────────────────────────────────────────────────
outpath = FIG_DIR / "top10_pairs.pdf"
