"""
Representative RQVI-Flashier pairs spanning the full correlation spectrum.

Shows 6 pairs from r~1.0 down to r~0.37, visually calibrating what different
correlation values mean biologically. Pairs #4 and #5 bracket the r=0.5
threshold used in the coverage analysis, making the key claim concrete:
pairs above r>=0.5 show genuine biological agreement.

Layout: gridspec(7, 4)
  - Rows 0-5: one pair each -> [RQVI UMAP, Flashier UMAP, RQVI MD, Flashier MD]
  - Row 6: histogram spanning all 4 columns, with r>=0.5 region shaded
"""
...
# ─── Hardcoded 6 pairs spanning the correlation spectrum ─────────────────────
PAIRS = [
    {"flashier_factor": "F21",  "rqvi_gp": 60, "best_corr": 0.991},
    {"flashier_factor": "F62",  "rqvi_gp": 55, "best_corr": 0.817},
    {"flashier_factor": "F197", "rqvi_gp": 21, "best_corr": 0.721},
    {"flashier_factor": "F58",  "rqvi_gp": 38, "best_corr": 0.573},  # was F193/GP30
    {"flashier_factor": "F35",  "rqvi_gp": 45, "best_corr": 0.436},  # was F194/GP63
    {"flashier_factor": "F178", "rqvi_gp": 33, "best_corr": 0.373},
]

# Tier labels for left-side annotation
TIER_LABELS = [
    "Near-perfect",
    "Strong",
    "Good",
    "Above threshold (r\u22650.5)",
    "Below threshold",
    "Clearly different",
]
...
print(f"Selected {len(PAIRS)} pairs spanning r={PAIRS[0]['best_corr']:.3f} to {PAIRS[-1]['best_corr']:.3f}")
...
# Compute Flashier cluster means now (needed for histogram later, before flashier_cell is deleted)
print("Computing Flashier cluster means for histogram...")
...
# ─── Step 5: Extract RQVI & Flashier loadings for all pairs ─────────────────
...
# ─── Step 7: Compute multi-seed best-corr distribution (matching coverage figure) ──
print("Computing multi-seed best-match correlations for histogram...")
N_SEEDS = 10
...
# Max across all seeds = effective best match with full RQVI repertoire
...
for row_i, p in enumerate(PAIRS):
    ...
    # Left-side row label: correlation value + tier
    ax.set_ylabel(
        f"r = {p['best_corr']:.3f}\n{TIER_LABELS[row_i]}",
...
# Shade the r>=0.5 region
ax_hist.axvspan(0.5, 1.0, color="#d4edda", alpha=0.5, zorder=0,
                label="r \u2265 0.5 region")

# Draw histogram
ax_hist.hist(best_corr_all_seeds, bins=30,
             color="#4C72B0", edgecolor="white", linewidth=0.5, alpha=0.7,
             zorder=2)

# Mark the 6 selected pairs with dashed vertical lines
colors_6 = ["#e41a1c", "#ff7f00", "#984ea3", "#4daf4a", "#377eb8", "#a65628"]
for i, p in enumerate(PAIRS):
    ax_hist.axvline(
        p["best_corr"], color=colors_6[i], linestyle="--", linewidth=1.5,
        label=f"GP {p['rqvi_gp']} vs {p['flashier_factor']} (r={p['best_corr']:.2f})",
        zorder=3,
    )

# r=0.5 threshold line
ax_hist.axvline(0.5, color="black", linestyle="-", linewidth=1.5, zorder=4)

# Annotation in the shaded region
...
ax_hist.set_xlabel("Best-match |Pearson r| (cluster-level, 10 seeds)", fontsize=9)
ax_hist.set_ylabel("Count (Flashier factors)", fontsize=9)
ax_hist.set_title(
    "Cluster-level similarity distribution (10 seeds) \u2014 6 representative pairs marked",
...
# ─── Save ────────────────────────────────────────────────────────────────────
outpath = FIG_DIR / "representative_pairs.pdf"
