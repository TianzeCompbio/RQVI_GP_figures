"""
Panel E (GP37 vs F32): UMAP cell-loading maps + MD gene-effect plots for pair 1.

2×2 layout: top row = UMAP (RQVI GP37, Flashier F32),
            bottom row = MD (RQVI GP37, Flashier F32).
"""
...
from utils import (
    load_main_obs, load_umap_coords, load_gene_effects,
    FIG_DIR, PATH_RQVI_H5AD, PATH_MAIN_H5AD,
    LEVEL1_ORDER,
)

# ─── Config ───────────────────────────────────────────────────────────────────
PAIR = {"rqvi_gp": 37, "flashier_factor": 32}
...
flashier_loading = flashier_cell.loc[sampled_idx, col].values
...
# RQVI UMAP
ax = axes[0, 0]
vals_u = rqvi_loading[shuffle_order]
vmax = np.percentile(vals_u[vals_u > 0], 99) if (vals_u > 0).any() else 1.0
ax.scatter(
    umap_coords[shuffle_order, 0], umap_coords[shuffle_order, 1],
    s=0.3, c=vals_u, cmap="viridis", vmin=0, vmax=vmax,
    alpha=0.6, rasterized=True,
)
ax.set_title(f"RQVI GP {PAIR['rqvi_gp']}", fontsize=9, fontweight="bold")
...
# Flashier UMAP
ax = axes[0, 1]
vals_u = flashier_loading[shuffle_order]
vmax = np.percentile(vals_u[vals_u > 0], 99) if (vals_u > 0).any() else 1.0
ax.scatter(
    umap_coords[shuffle_order, 0], umap_coords[shuffle_order, 1],
    s=0.3, c=vals_u, cmap="viridis", vmin=0, vmax=vmax,
...
ax.set_title(f"Flashier F{PAIR['flashier_factor']}", fontsize=9, fontweight="bold")
ax.set_xticks([]); ax.set_yticks([])
for spine in ax.spines.values():
    spine.set_visible(False)
...
fig.suptitle("GP37 vs F32", fontsize=12, fontweight="bold", y=1.02)
fig.tight_layout()

# ─── Save ─────────────────────────────────────────────────────────────────────
outpath = FIG_DIR / "panel_E_pair1.pdf"
