...
> **Main figure panel a.** Hand-drawn schematic that introduces the RQVI model and its post-training inference procedure. Sets up the two output matrices (cell × GP loadings and GP × gene effects) used in every other panel. See the panel-level storyline in [`figures_summary.md`](figures_summary.md).
