...
| Aligned EBMF–RQVI cluster comparison | 200 EBMF factors and 200 distinct matches from 10 pooled RQVI seeds across the same 114 clusters | `fig_ebmf_rqvi_level2_comparison.py` | `figures/main_figures/ebmf_rqvi_level2_comparison.pdf` | [`docs/ebmf_rqvi_level2_comparison.md`](docs/ebmf_rqvi_level2_comparison.md) |
...
The level2-cluster heatmap is self-contained: its losslessly compressed source matrix is included as `data/rqvi_cell_loadings_seed0.h5`, with exact cell names, program names/indices, and cluster annotations. The aligned EBMF–RQVI comparison can also be redrawn without external data because its two final 200 × 114 display matrices are included under `data/`.

### External data

Most scripts depend on large external files. The level2-cluster heatmap and the default redraw of the aligned comparison use data bundled in this repository. External inputs are needed only when recomputing the comparison matching or running the other analyses.
...
| `.../Evaluation/function_analysis/corr_rst/rqvi_seed{0-9}_gp_cell_level.csv` | RQVI cluster-level mean loadings (all 10 seeds) | Aligned comparison with `--recompute-matches`, sparsity scatter, correlation histogram, coverage |
...
python scripts/fig_ebmf_rqvi_level2_comparison.py # redraw from saved 200 x 114 display matrices
